...
export default function LinkChild() {
  ...
  const [success, setSuccess] = useState(null); // { firstName, lastName }
  const [error, setError] = useState('');

  // If not logged in, redirect to login with return URL
  useEffect(() => {
    if (!user) {
      navigate(`/login?redirect=${encodeURIComponent(`/link?school=${schoolSlug}&code=${code}`)}`);
    }
  }, [user, navigate, code]);
  ...
  const handleLink = async () => {
    ...
    setError('');
    try {
      // If school slug provided, join the school first and resolve schoolId
      let schoolId = null;
      if (schoolSlug) {
