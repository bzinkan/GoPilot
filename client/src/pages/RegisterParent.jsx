...
export default function RegisterParent() {
  const { registerParent, loginWithGoogle } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await registerParent({ firstName: form.firstName, lastName: form.lastName, email: form.email, password: form.password, phone: form.phone });
      navigate(redirectTo || '/parent');
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Create your parent account</p>
        </div>
        ...
        <p className="text-center mt-6 text-gray-500">
          ...
          <Link to={redirectTo ? `/login?redirect=${encodeURIComponent(redirectTo)}` : '/login'} className="text-blue-600 font-medium hover:underline">Sign in</Link>
        </p>

        <p className="text-center mt-2 text-gray-400 text-sm">
          Are you a school admin?{' '}
          <Link to="/register" className="text-blue-600 hover:underline">Register your school</Link>
        </p>
      </div>
    </div>
  );
