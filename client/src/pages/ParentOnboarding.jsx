...
import { getChildren, linkChild, addPickup, getPickups } from '../api/pickups';
...
export default function ParentOnboarding() {
  ...
  // Start at step 1 (confirmation) since user is already authenticated
  ...
  // Fetch linked children from API
  ...
  // Fetch existing pickups for all children
  const fetchPickups = useCallback(async (childList) => {
    try {
      const allPickups = [];
      for (const child of childList) {
        const res = await getPickups(child.id);
        if (res.data) {
  ...
  // Load children on mount and when entering step 2
  useEffect(() => {
    fetchChildren();
  }, [fetchChildren]);

  // When children are loaded, fetch their pickups
  useEffect(() => {
  ...
  // Add child by student code via API
  const handleAddChildByCode = async (code) => {
    setIsLoading(true);
    setError(null);
    try {
      await linkChild({ student_code: code });
      await fetchChildren();
    } catch (err) {
  ...
  // Add authorized pickup via API for each linked child
  const handleAddAuthorizedPickup = async (pickup) => {
    ...
    try {
      ...
      for (const child of children) {
        const res = await addPickup(child.id, pickup);
  ...
  // Remove authorized pickup (local only - remove from display)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white">
      ...
      <main className="max-w-lg mx-auto px-4 py-8">
        ...
        {/* Step 1: Account Confirmation (already signed in) */}
        {currentStep === 1 && (
        ...
        {currentStep === 5 && (
          <div className="space-y-6">
            ...
            <Button variant="primary" size="lg" className="w-full" onClick={() => navigate('/parent')}>
