...
import {
  getOrCreateSession,
  getQueue,
  releaseStudent as releaseStudentApi,
  releaseBatch as releaseBatchApi,
} from '../api/dismissal';
...
export default function TeacherView() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const homeroomsRes = await getHomerooms(currentSchool.id);
        ...
        const studentsRes = await getStudents(currentSchool.id, { homeroomId: myHomeroom.id });
        const sessionRes = await getOrCreateSession(currentSchool.id);
        const sessionData = sessionRes.data;
        if (!cancelled) setSession(sessionData);

        const queueRes = await getQueue(sessionData.id, { homeroomId: myHomeroom.id });
  ...
  useEffect(() => {
    ...
    const handleQueueUpdated = async () => {
      ...
      try {
        const queueRes = await getQueue(session.id, { homeroomId: homeroom.id });
        const queueItems = Array.isArray(queueRes.data) ? queueRes.data : queueRes.data?.items || [];
        const queueByStudentId = {};
        queueItems.forEach((q) => {
          queueByStudentId[q.student_id || q.studentId] = q;
  ...
  const handleDismissFromClass = async (student) => {
    ...
    try {
      await releaseStudentApi(student.queueId);
      setStudents((prev) =>
        prev.map((s) =>
  ...
  const handleDismissAll = async (studentList) => {
    ...
    try {
      await releaseBatchApi({ queueIds });
      setStudents((prev) =>
        prev.map((s) =>
